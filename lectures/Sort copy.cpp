...
template <class T, class Filter, class PSort, class SSort>
void filter_sort(T& mas, Filter filter, PSort psort, SSort ssort) {
    auto it = std::remove_if(mas.begin(), mas.end(), [filter](auto a) { return !filter(a); });

    std::stable_sort(mas.begin(), it, [ssort](auto a, auto b) { return ssort(a, b); });
    std::stable_sort(mas.begin(), it, [psort](auto a, auto b) { return psort(a, b); });

    mas.erase(it, mas.end());
}
